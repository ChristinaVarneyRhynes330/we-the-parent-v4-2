...
export default function EventForm({ isOpen, onClose, onEventAdded, editingEvent }: EventFormProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTimeout(() => modalRef.current?.focus(), 100); // Defer focus
  ...
  const resetForm = (isSubmitSuccess = false) => {
    if (!isSubmitSuccess) {
        setFormData({
        title: '',
      ...
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-screen overflow-y-auto">
          ...
          {!editingEvent && (
            <div className="mt-4">
              <button
                ...
                aria-controls="quick-add-region"
        ...
        <div className="p-6">
          {!editingEvent && showQuickAdd && (
            <div id="quick-add-region" className="mb-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <Icon aria-hidden="true" className={`w-4 h-4 text-${eventType?.color}`} />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input id="title" name="title" value={formData.title} onChange={handleInputChange} className="form-input" placeholder="Enter event title..." required />
              </div>
            ...
            {selectedEventType && (
              <div className={`p-3 rounded-lg bg-${selectedEventType.color}/10 border border-${selectedEventType.color}/20`}>
                <div className="flex items-center gap-2">
                  <selectedEventType.icon aria-hidden="true" className={`w-5 h-5 text-${selectedEventType.color}`} />
                  <span className={`text-${selectedEventType.color} font-medium`}>{selectedEventType.label}</span>
                </div>
              </div>
            )}

            <div>
              <label htmlFor="event_date" className="form-label">Date and Time *</label>
              <div className="relative">
                <Calendar aria-hidden="true" className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                <input id="event_date" type="datetime-local" name="event_date" value={formData.event_date || getDefaultDate()} onChange={handleInputChange} className="form-input pl-10" required />
              </div>
              <p className="text-xs text-slate-gray mt-1">Select the date and time for this event</p>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
              <button type="button" onClick={() => { resetForm(); onClose(); }} className="button-secondary" disabled={isSubmitting}>Cancel</button>
              {!editingEvent && (
                <button type="button" onClick={() => resetForm()} className="button-outline" disabled={isSubmitting}>Clear Form</button>
              )}
